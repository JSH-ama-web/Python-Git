...
![프로그램 메인 화면 이미지 삽입](경로를_입력해주세요/main_screen.png)
> *여기에 터미널 실행 초기 화면 스크린샷을 삽입해주세요.*
...
> *명령어 입력 후 프로그램이 구동되는 화면 스크린샷을 추가해주세요.*
...
### 📝 텍스트 생성
* 텍스트 초안 작성, 요약, 번역 등 텍스트 결과물을 도출하기 위한 프롬프트를 저장합니다.

### 🎨 이미지 생성
* DALL-E, Midjourney 등의 AI 도구를 활용하여 원하는 이미지를 생성하기 위한 구체적인 묘사나 스타일 프롬프트를 저장합니다.

### 🎬 영상 생성
* AI 영상 생성 도구에 사용할 씬(Scene) 묘사, 화면 전환, 카메라 워킹 등의 프롬프트를 관리합니다.

### 🎭 페르소나
* AI에게 특정 직업이나 성격을 부여하여 그 역할에 맞게 답변하도록 유도하는 프롬프트를 관리합니다.
...
![카테고리 화면 이미지 삽입](경로를_입력해주세요/categories_screen.png)
> *카테고리별로 정렬된 프롬프트 목록 스크린샷을 추가해주세요.*

---
*이 README 문서는 필요에 따라 지속적으로 업데이트될 예정입니다.*
